 ...
 *               <p>
 *               This sample demonstrates how to build your own Receiver for use
 *               with Google Cast. One of the goals of this sample is to be
 *               fully UX compliant.
 *               </p>
 *               <p>
 *               A receiver is typically an HTML5 application with a html, css,
 *               and JavaScript components. It demonstrates the following Cast
 *               Receiver API's:
 *               </p>
 *               <ul>
...
var licenseurl = "https://widevinet.video.friday.tw/?deviceId=Y2ZjMWM1NzAtM2RhZC0zOTgwLTg5YjgtYTdjOTBiNzljMjM0"
var customMessageBus;
var customMessageBusEventSenderId;
/**
 * <p>
 ...
 * error, stalled, waiting, playing, pause, ended, timeupdate, seeking, & seeked</li>
 * <li>Find and remember the various elements</li>
 * <li>Create the MediaManager and bind to onLoad & onStop</li>
 * </ol>
 *
 * @param {!Element}
 *            element the element to attach the player
 * @struct
 * @constructor
 * @export
 */
sampleplayer.CastPlayer = function(element) {
	 ...
	 * @private {!Element}
	 */
	this.progressBarInnerElement_ = this
			.getElementByClass_('.controls-progress-inner');
	 ...
	 * @private {!Element}
	 */
	this.progressBarThumbElement_ = this
			.getElementByClass_('.controls-progress-thumb');
	 ...
	 * @private {!Element}
	 */
	this.previewModeTimerElement_ = this
			.getElementByClass_('.preview-mode-timer-countdown');
	...
	this.mediaElement_.addEventListener('error', this.onError_.bind(this),
			false);
	this.mediaElement_.addEventListener('playing', this.onPlaying_.bind(this),
			false);
	this.mediaElement_.addEventListener('pause', this.onPause_.bind(this),
			false);
	this.mediaElement_.addEventListener('ended', this.onEnded_.bind(this),
			false);
	this.mediaElement_.addEventListener('abort', this.onAbort_.bind(this),
			false);
	this.mediaElement_.addEventListener('timeupdate', this.onProgress_
			.bind(this), false);
	this.mediaElement_.addEventListener('seeking',
			this.onSeekStart_.bind(this), false);
	 ...
	 *
	 * @private {!cast.receiver.CastReceiverManager}f
	 */

	this.receiverManager_ = cast.receiver.CastReceiverManager.getInstance();
	this.receiverManager_.onReady = this.onReady_.bind(this);
	this.receiverManager_.onSenderDisconnected = this.onSenderDisconnected_
			.bind(this);
	this.receiverManager_.onVisibilityChanged = this.onVisibilityChanged_
			.bind(this);
	this.receiverManager_.setApplicationState(sampleplayer
			.getApplicationState_());



    // modify by jackie
    // 從chromecast -> receiverManager 元件中,加入 onMessage 事件
    // 可以接收來自於 sender 發送出之訊息.
    // 目前規則建議.發送之訊息皆以JSON格式傳送.以 MessageType 作為資料傳遞之型態規範
    // 使用於變更字幕範例 '{"MessageType": "subtitle",  "Name": "tw",  "URI": "http://220.134.220.142:8888/srt/webvtt09.vtt",  "Language": "Chinese-tw" }'
    customMessageBus = this.receiverManager_.getCastMessageBus('urn:x-cast:tw.friday.video.cast');
    customMessageBus.onMessage = function(event) 
    {
        customMessageBusEventSenderId = event.senderId;
        // Handle message
        //console.log('◣◣◣◣◣customMessageBus.onMessage');
        //console.log('Message [' + event.senderId + ']: ' + event.data);
    
        // 將收到的資料傳給receiverData()做處理
        
        receiverData(event.data);
	 ...
	 */
	this.onLoadOrig_ = this.mediaManager_.onLoad.bind(this.mediaManager_);
	 ...
	 * @private {?function(!cast.receiver.MediaManager.Event)}
	 */
	this.onEditTracksInfoOrig_ = this.mediaManager_.onEditTracksInfo
			.bind(this.mediaManager_);
	 ...
	 */
	this.onMetadataLoadedOrig_ = this.mediaManager_.onMetadataLoaded
			.bind(this.mediaManager_);
	 ...
	 * @private {?function(cast.receiver.MediaManager.Event)}
	 */
	this.onStopOrig_ = this.mediaManager_.onStop.bind(this.mediaManager_);
	 ...
	 * @private {?function(!cast.receiver.MediaManager.LoadInfo)}
	 */
	this.onLoadMetadataErrorOrig_ = this.mediaManager_.onLoadMetadataError
			.bind(this.mediaManager_);
	this.mediaManager_.onLoadMetadataError = this.onLoadMetadataError_
			.bind(this);
	...
	this.mediaManager_.customizedStatusCallback = this.customizedStatusCallback_
			.bind(this);
 ...
 *
 * @param {string}
 *            className The class name of the element to return.
 * @return {!Element}
 * @throws {Error}
 *             If given class cannot be found.
 * @private
 */
 ...
 *
 * @param {!cast.receiver.media.MediaInformation}
 *            mediaInformation The asset media information.
 ...
 *
 * @param {boolean}
 *            show whether player is showing preview mode metadata
 * @export
 */
...
sampleplayer.CastPlayer.prototype.preloadVideo_ = function(mediaInformation) {
	processAnalytics();
	createInfo(this.receiverManager_.getApplicationData().sessionId);
	if (isLaunching) {
		isLaunching = false;
		var i = 0;
	...
	addAnalyticsData('"t":"open","ts":' + new Date().getTime() + ',"uri":"'
			+ info.message.media.contentId + '"');
	this.log_('preloadVideo_');
	var self = this;
	var subUrl = getUrl(info.message.media.contentId);
	if (subUrl != null) {
		info.message.media.contentId = subUrl;
	}
	...
	host.onError = function(errorCode) {
		...
		message += (currentTime * 1000);
		message += ',"rsn":"';
		message += errorCode;
		message += '"';
		addAnalyticsData(message);
 ...
 *
 * @param {!cast.receiver.MediaManager.LoadInfo}
 *            info The load request info.
 * @export
 */
...
//        console.log("▉▉▉self.loadMetadata_(media);=======================================================▉▉▉");
//        console.log("▉▉▉:"+media);
//        console.log("▉▉▉=======================================================▉▉▉");
		...
		sampleplayer
				.preload_(
						...
						function() {
							...
							if (preloaded) {
								// Data is ready to play so transiton directly
								// to playing.
								self.setState_(sampleplayer.State.PLAYING,
										false);
								self.playerReady_ = true;
								self.maybeSendLoadCompleted_(info);
								// Don't display metadata again, since autoplay
								// already did that.
								self.deferPlay_(0);
								...
								sampleplayer
										.transition_(
												self.element_,
												sampleplayer.TRANSITION_DURATION_,
												function() {
													self
															.setState_(
																	sampleplayer.State.LOADING,
																	false);
													// Only send load completed
													// after we reach this point
													// so the media
													// manager state is still
													// loading and the sender
													// can't send any PLAY
													// messages
													self.playerReady_ = true;
													self
													...
													if (self.playerAutoPlay_) {
														// Make sure media info
														// is displayed long
														// enough before
														// playback
														// starts.
														self
																.deferPlay_(sampleplayer.MEDIA_INFO_DURATION_);
														self.playerAutoPlay_ = false;
													}
												});
							}
...
//        console.log("▉▉▉sampleplayer.CastPlayer.prototype.loadMetadata_ = function(media) {=======================================================▉▉▉");
//        console.log("▉▉▉ titleElement:"+titleElement);
//        console.log("▉▉▉ subtitleElement:"+subtitleElement);
//        console.log("▉▉▉ artwork:"+artwork);
//        console.log("▉▉▉ xxxx:"+media);
//        console.log("▉▉▉=======================================================▉▉▉");
...
sampleplayer.CastPlayer.prototype.loadPreviewModeMetadata_ = function(media) {
	...
	if (!sampleplayer.isCastForAudioDevice_()) {
		...
		var subtitleElement = this.element_
				.querySelector('.preview-mode-subtitle');
		sampleplayer.setInnerText_(subtitleElement, metadata.subtitle);

		var artwork = sampleplayer.getMediaImageUrl_(media);
		if (artwork) {
			var artworkElement = this.element_
					.querySelector('.preview-mode-artwork');
			sampleplayer.setBackgroundImage_(artworkElement, artwork);
		}
	}
};
...
/**
 * Lets player handle autoplay, instead of depending on underlying MediaElement
 * to handle it. By this way, we can make sure that media playback starts after
...
sampleplayer.CastPlayer.prototype.loadVideo_ = function(info) {
	...
	if (!protocolFunc) {
		...
		// getState()['underflow]'
		this.mediaElement_.removeEventListener('stalled',
				this.bufferingHandler_);
		this.mediaElement_.removeEventListener('waiting',
				this.bufferingHandler_);
		...
		if (!this.preloadPlayer_
				|| (this.preloadPlayer_.getHost && this.preloadPlayer_
						.getHost().url != url)) {
			if (this.preloadPlayer_) {
				this.preloadPlayer_.unload();
				this.preloadPlayer_ = null;
			...
			this.preloadPlayer_ = null;
			// Replace the "preload" error callback with the "load" error
			// callback
 ...
 *            info The load request info.
 * @param {boolean}
 *            loadOnlyTracksMetadata Only load the tracks metadata (if it is in
 *            the info provided).
 * @private
 */
sampleplayer.CastPlayer.prototype.loadMediaManagerInfo_ = function(info,
		loadOnlyTracksMetadata) {
	...
	if (loadOnlyTracksMetadata) {
		...
		// Media supported by mediamanager, use the media manager default onLoad
		// API
		// to load the media, tracks metadata and, if the tracks are vtt the
		// media
...
sampleplayer.CastPlayer.prototype.readSideLoadedTextTrackType_ = function(info) {
	...
	for (var i = 0; i < info.message.media.tracks.length; i++) {
		...
		if (this.isTtmlTrack_(info.message.media.tracks[i])) {
			this.textTrackType_ = sampleplayer.TextTrackType.SIDE_LOADED_TTML;
		} else if (this.isVttTrack_(info.message.media.tracks[i])) {
			this.textTrackType_ = sampleplayer.TextTrackType.SIDE_LOADED_VTT;
		} else {
			this.log_('Unsupported side loaded text track types');
			this.textTrackType_ = sampleplayer.TextTrackType.SIDE_LOADED_UNSUPPORTED;
			break;
		}
		// We do not support text tracks with different caption types for a
		// single
		// piece of content
 ...
 * @private
 */
sampleplayer.CastPlayer.prototype.maybeLoadSideLoadedTracksMetadata_ = function(
		info) {
...
/**
 * Loads embedded tracks information without loading media. If there is embedded
 * tracks information, it loads the tracks information in the media manager
 ...
 * @private
 */
sampleplayer.CastPlayer.prototype.maybeLoadEmbeddedTracksMetadata_ = function(
		info) {
 ...
 *
 * @param {!Array.
 *            <number>} activeTrackIds The active tracks.
 * @param {!Array.
 *            <cast.receiver.media.Track>} tracks The track definitions.
 * @private
 */
sampleplayer.CastPlayer.prototype.processTtmlCues_ = function(activeTrackIds,
		tracks) {
	...
	for (var i = 0; i < tracks.length; i++) {
		...
		if (!this.player_) {
			...
			// host.licenseUrl =
			// "http://203.69.207.187:8080/?deviceId=YmViZDY2YjktMjhhMi0zNWZlLThhMDktODU2NjVjNDQzYWE3";
			this.protocol_ = null;
			this.player_ = new cast.player.api.Player(host);
		}
		this.player_.enableCaptions(true, cast.player.api.CaptionsType.TTML,
				tracks[i].trackContentId);
	}
};
 ...
 *
 * @param {cast.receiver.media.Track}
 *            track The track.
 ...
 * @param {cast.receiver.media.Track}
 *            track The track.
 * @param {!sampleplayer.TextTrackType}
 *            textTrackType The text track type expected.
 * @param {!string}
 *            mimeType The mimeType expected.
 * @return {boolean} Whether the track has the specified format.
 * @private
 */
sampleplayer.CastPlayer.prototype.isKnownTextTrack_ = function(track,
		textTrackType, mimeType) {
	if (!track) {
		return false;
	}
 ...
 * @private
 */
sampleplayer.CastPlayer.prototype.processInBandTracks_ = function(
		activeTrackIds) {
	var isReload = false;
	...
	for (var i = 0; i < streamCount; i++) {
		...
		if (isActive && !wasActive) {
			if (mediaInformation.tracks[i].type == 'AUDIO')
				isReload = true;
 ...
 *
 * @param {number}
 *            t the time in milliseconds before the player goes idle
 * @private
 */
 ...
 *
 * @param {sampleplayer.Type}
 *            type The type of player.
 * @param {boolean}
 *            isLiveStream whether player is showing live content
 * @private
 */
sampleplayer.CastPlayer.prototype.setType_ = function(type, isLiveStream) {
	...
	if (type != sampleplayer.Type.AUDIO) {
		...
		// prevent screen burn
		this.burnInPreventionIntervalId_ = setInterval(
				function() {
					overlay.style.marginBottom = Math
							.round(Math.random() * 100)
							+ 'px';
					overlay.style.marginLeft = Math.round(Math.random() * 600)
							+ 'px';
 ...
 *
 * @param {sampleplayer.State}
 *            state the new state of the player
 * @param {boolean=}
 *            opt_crossfade true if should cross fade between states
 * @param {number=}
 *            opt_delay the amount of time (in ms) to wait
 * @private
 */
sampleplayer.CastPlayer.prototype.setState_ = function(state, opt_crossfade,
		opt_delay) {
	...
	if (analyticsState != state) {
		...
		message += state;
		message += '","ts":';
		message += self.lastStateTransitionTime_;
		message += ',"ps":';
		var currentTime = 0.0;
		if (this.mediaElement_) {
			currentTime = this.mediaElement_.currentTime;
		...
		if (!isLaunching) {
			...
			launchingString[launchingString.length] = message;
		}
	}
	...
	if (opt_delay) {
		...
		if (!opt_crossfade) {
			...
			self
					.setIdleTimeout_(sampleplayer.IDLE_TIMEOUT[state
							.toUpperCase()]);
		} else {
			...
			sampleplayer
					.transition_(
							...
							sampleplayer.TRANSITION_DURATION_,
							function() {
								// In the case of a crossfade transition, the
								// transition will be completed
								// even if setState is called during the
								// transition. We need to be sure
								// that the requested state is ignored as the
								// latest setState call should
								// take precedence.
								if (stateTransitionTime < self.lastStateTransitionTime_) {
									self
											.log_('discarded obsolete deferred state('
													+ state + ').');
									return;
								}
...
/**
 * Called when the player is ready. We initialize the UI for the launching and
 * idle screens.
 *
 * @private
 */
 ...
 *
 * @param {cast.receiver.CastReceiverManager.SenderDisconnectedEvent}
 *            event
 * @private
 */
...
/**
 * Called when media has an error. Transitions to IDLE state and calls to the
 * original media manager implementation.
 *
...
/**
 * Called when media is buffering. If we were previously playing, transition to
 * the BUFFERING state.
 *
 * @private
 */
...
/**
 * Called when media has started playing. We transition to the PLAYING state.
 *
 * @private
 */
 ...
 *
 * @param {!cast.receiver.media.MediaStatus}
 *            mediaStatus Media status that is supposed to go to sender.
...
/**
 * Called when we receive a STOP message. We stop the media and transition to
 * the IDLE state.
 *
 * @param {cast.receiver.MediaManager.Event}
...
sampleplayer.CastPlayer.prototype.onEnded_ = function() {
	...
	addAnalyticsData('"t":"end","ts":' + new Date().getTime() + ',"ps":"'
			+ (currentTime * 1000));
	this.log_('onEnded');
...
sampleplayer.CastPlayer.prototype.updateProgress_ = function() {
	...
	if (!sampleplayer.isCastForAudioDevice_()) {
		...
		if (!isNaN(curTime) && !isNaN(totalTime)) {
			...
			this.curTimeElement_.innerText = sampleplayer
					.formatDuration_(curTime);
			this.totalTimeElement_.innerText = sampleplayer
			...
			if (this.displayPreviewMode_) {
				this.previewModeTimerElement_.innerText = ""
						+ Math.round(totalTime - curTime);
			}
		}
	}
};
...
sampleplayer.CastPlayer.prototype.onSeekStart_ = function() {
	...
	addAnalyticsData('"t":"seek","ts":' + new Date().getTime() + ',"ps":"'
			+ (currentTime * 1000));
...
/**
 * Called when the player is added/removed from the screen because HDMI input
 * has changed. If we were playing but no longer visible, pause the currently
 * playing media.
 *
 * @see cast.receiver.CastReceiverManager#onVisibilityChanged
 * @param {!cast.receiver.CastReceiverManager.VisibilityChangedEvent}
 *            event Event fired when visibility of application is changed.
 * @private
 */
 ...
 *
 * @param {!cast.receiver.MediaManager.Event}
 *            event The editTracksInfo event.
 * @private
 */
	...
	if (type == sampleplayer.TextTrackType.SIDE_LOADED_TTML) {
		...
		if (this.player_) {
			this.player_.enableCaptions(false,
					cast.player.api.CaptionsType.TTML);
		}
...
sampleplayer.CastPlayer.prototype.onMetadataLoaded_ = function(info) {
	...
	if (this.textTrackType_ == sampleplayer.TextTrackType.SIDE_LOADED_TTML
			&& info.message && info.message.activeTrackIds
			&& info.message.media && info.message.media.tracks) {
		this.processTtmlCues_(info.message.activeTrackIds,
...
/**
 * Called when the media could not be successfully loaded. Transitions to IDLE
 * state and calls the original media manager implementation.
 *
 * @see cast.receiver.MediaManager#onLoadMetadataError
 * @param {!cast.receiver.MediaManager.LoadInfo}
 *            event The data associated with a LOAD event.
 * @private
 */
...
sampleplayer.CastPlayer.prototype.onLoadSuccess_ = function() {
	...
	var totalTime = this.mediaElement_.duration;
	if (!isNaN(totalTime)) {
		this.totalTimeElement_.textContent = sampleplayer
				.formatDuration_(totalTime);
	} else {
 ...
 * @param {!cast.receiver.media.MediaInformation}
 *            mediaInformation The asset media information.
 * @return {?function(cast.player.api.Host):player.StreamingProtocol} The
 *         protocol function that corresponds to this media type.
 * @private
 */
...
/**
 * Returns true if the preview UI should be shown for the type of media although
 * the media can not be preloaded.
 *
 * @param {!cast.receiver.media.MediaInformation}
 *            media The media information.
...
/**
 * Returns the type of player to use for the given media. By default this looks
 * at the media's content type, but falls back to file extension if not set.
 *
 * @param {!cast.receiver.media.MediaInformation}
 *            media The media.
 ...
 *
 * @param {!Element}
 *            element The element to add the given class.
 * @param {string}
 *            className The class name to add to the given element.
 * @param {number}
 *            timeout The amount of time (in ms) the class should be added to
 *            the given element.
...
/**
 * Causes the given element to fade out, does something, and then fades it back
 * in.
 *
 * @param {!Element}
 *            element The element to fade in/out.
 * @param {number}
 *            time The total amount of time (in seconds) to transition.
 * @param {function()}
 *            something The function that does something.
 * @private
 */
 ...
 *
 * @param {!cast.receiver.media.MediaInformation}
 *            media The media to load.
 * @param {function()}
 *            doneFunc The function to call when done.
 * @private
 */
 ...
 * @param {number}
 *            time The amount of time (in seconds) to transition.
 * @param {function()=}
 *            opt_doneFunc The function to call when complete.
 * @private
 */
...
/**
 * Causes the given element to fade to the given opacity in the given amount of
 * time.
 *
 * @param {!Element}
 *            element The element to fade in/out.
 * @param {string|number}
 *            opacity The opacity to transition to.
 * @param {number}
 *            time The amount of time (in seconds) to transition.
 * @param {function()=}
 *            opt_doneFunc The function to call when complete.
 ...
 *
 * @param {cast.receiver.media.MediaInformation=}
 *            opt_media The current media metadata
...
sampleplayer.setBackgroundImage_ = function(element, opt_url) {
	...
	element.style.backgroundImage = (opt_url ? 'url("'
			+ opt_url.replace(/"/g, '\\"') + '")' : 'none');
...
sampleplayer.isCastForAudioDevice_ = function() {
	var receiverManager = window.cast.receiver.CastReceiverManager
			.getInstance();
	if (receiverManager) {
...
function getUrl(url) {
	...
	if (xmlhttp.status === 200) {
		...
		try {
			...
			var pattern = /Location>(.+)</g;
			var str2 = str.match(pattern);
			if (str2 != null) {
				str = str2[0];
			...
			sampleplayer.CastPlayer.prototype.log_('parser error:' + err);
		}
	}
	return null;
}